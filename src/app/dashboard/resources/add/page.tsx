...
import { createResourcesServices } from "@/services/resourceServices";
...
const PageAddResource = () => {
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    setIsLoading(true);
    try {
      await createResourcesServices(data);
    ...
    <div className="p-5 min-h-full">
      <div className="flex flex-1 flex-col space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h2 className="text-3xl font-bold tracking-tight">Add Resource</h2>
            <p className="text-sm text-muted-foreground">
              Fill data and submit new Resource
            </p>
          </div>
        </div>
      </div>
